...
3. Record only a neutral capability name, availability, reference support, dimension
...
4. Make no speculative network request for detection. The first canonical-reference
   generation is the operational check.
5. After each invocation, require a local PNG/JPEG/WebP that Pillow can read, with both
